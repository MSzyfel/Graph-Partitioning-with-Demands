...
\]
This problem is also well-understood, \DD{tutaj zmienilem poczatek zdania; do weryfikacji czy zachowuje sens?} and the state-of-the-art approximation algorithm is due to Arora, Lee, and Naor \cite{EuclideanDistortionAndTheSparsestCut}, which achieves the ratio of $\bigo\br{\sqrt{\log n}\cdot \log \log n}$. However, as discussed above, in order to use sparsest cut in a wider range of applications, it would be desirable to approximate some quantity measuring the balance of a cut.
To the best of our knowledge, no such results have been obtained in the literature yet. To address this gap, we introduce the notion of \emph{generalized conductance} of a cut $\br{S,\bar{S}}$, defined as
\[
