...
We introduced and studied the \textsc{Generalized Conductance}, a problem of interest on its own, and also important from the perspective of potential applications. Our main algorithm combines two complementary reductions which yield an $\bigo\br{\log n}$-approximation on general graphs and an $\bigo\br{1}$-approximation on trees. For multiplicative demand functions, we further improve the approximation guarantee to $\bigo\br{\sqrt{\log n}}$.
...
\DD{SODA chce, aby references byly na koncu (?)}
